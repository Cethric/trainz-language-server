...
async fn bench_process_acs_text_file(
    ...
    server
        .process_acs_text_file(&path, content, false, &progress)
        .await;
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    path.push("sample.acs_text");

    let content = std::fs::read_to_string(&path).expect("Could not read test file");

    c.bench_function("process_acs_text_file sample", |b| {
        b.to_async(&rt)
            .iter(|| bench_process_acs_text_file(server, path.clone(), &content));
    });
}
