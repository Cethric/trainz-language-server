...
pub trait ProcessAcsText {
    fn process_acs_text_file(
        &self,
        path: &Path,
        content: &str,
        changed: bool,
...
impl ProcessAcsText for GameScriptLanguageServer {
    ...
    async fn process_acs_text_file(
        ...
        self.process_acs_text_file_inner(path, content, changed, progress)
            .await;
    }
}
    ...
    async fn process_acs_text_file_inner(
        ...
        if let Some(path_str) = path.to_str() {
            ...
            if let Ok(pairs) = pairs {
                ...
                let parsed = process_acs_text_ast(pairs, content);

                let parsed_arc = Arc::new(parsed);

                let comments_arc = match trainz_parser::comments::parse_acs_text_comments(content) {
                    Ok(mut pairs) => {
                ...
                self.parsed_files.insert(
                    path_str.to_string(),
                    ParsedFile {
                        parsed: ParsedFileType::AcsText(parsed_arc),
