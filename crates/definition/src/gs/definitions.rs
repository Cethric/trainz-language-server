...
fn find_member_type(
    ...
    while let Some(cls_name) = to_visit.pop() {
        ...
        if class_def.is_none() {
            if let Some((cls, _)) = parsed_files.par_iter().find_map_any(|entry| {
                entry
                    .value()
                    .classes
                    .iter()
...
fn infer_receiver_type(
    ...
    let mut current_type = match expr {
        ...
        Expr::NewObject { ty, .. } => match ty {
            Type::Named(id) => Some(id.name.clone()),
            _ => None,
        },
        _ => None,
    };
