...
export function activate(context: ExtensionContext) {
  ...
  function didOpenTextDocument(document: vscode.TextDocument) {
    ...
    if (!clients.has(folder.uri.toString())) {
      ...
      client.start().then(() => {
        // Set up diagnostics listener AFTER starting the client
        ...
        const disposable = workspace.onDidCloseTextDocument((document: vscode.TextDocument) => {
          diagnosticCollection.delete(document.uri);
        });
        context.subscriptions.push(disposable);

        registerLanguageFeatures(context, client);
      });
...
function convertDocumentSymbols(
  ...
  return symbols.map((symbol) => {
    ...
    if (symbol.children && Array.isArray(symbol.children)) {
      convertedSymbol.children = convertDocumentSymbols(symbol.children, client);
    }
 ...
 */
export function convertSymbolKind(lspKind: number): vscode.SymbolKind {
 ...
 */
function registerLanguageFeatures(context: ExtensionContext, client: LanguageClient) {
        ...
        provideCompletionItems: async (document, position, token) => {
          try {
            ...
            if (response.items && Array.isArray(response.items)) {
              return response.items.map((item: any) => convertCompletionItem(item));
            } else if (Array.isArray(response)) {
              return response.map((item: any) => convertCompletionItem(item));
            }
      ...
      ],
      {
        provideHover: async (document, position, token) => {
          try {
            ...
            // Handle both marked string and MarkupContent formats
            let contents: any[] = [];
      ...
      ],
      {
        provideDocumentSymbols: async (document, token) => {
          try {
            ...
            return convertDocumentSymbols(response, client);
          } catch (error) {
